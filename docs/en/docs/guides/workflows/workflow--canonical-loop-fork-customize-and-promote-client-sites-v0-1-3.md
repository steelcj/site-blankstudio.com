...
sat:changelog:
  - version: "0.1.2"
    ...
    notes: >
      Adopted the chart-heading rule from the mermaid-navigation style guide
      v0.1.3. The flowchart now sits under its own heading, The Canonical Loop
      Flowchart, matching the chart's accTitle, and the explicit
      canonical-loop-chart anchor is removed. The full description lives in the
      chart's accDescr only; the visible text under the heading is a short
      reading hint. Every return link now names the section in bold, as
      [Return to the **The Canonical Loop Flowchart** section]. Updated the
      accessibility notes to match.
  - version: "0.1.1"
    date: "2026-08-09"
    author: "Christopher Steel"
    notes: >
...
The flowchart below is clickable. Each box leads to the stage that describes it, and each stage links back to it. This document follows *Style Guide: Markdown Mermaid Navigation Flowcharts and Linked Sections* for that pattern.

## The Canonical Loop Flowchart

Read it clockwise from the top. Each box links to its stage below, and each stage links back here.

```mermaid
flowchart TD
    accTitle: The Canonical Loop Flowchart
...
[Return to the **The Canonical Loop Flowchart** section](#the-canonical-loop-flowchart)
...
Every stage boundary in this document is a real heading, never bold text, so a screen reader can build a section-by-section outline. Each box links to its heading's own auto-generated id, so a click lands on the heading itself. The flowchart is named and described for assistive technology through its `accTitle` and `accDescr`, which the renderer exposes as the SVG's name and description; the full description lives there rather than in a duplicate visible paragraph, so it is announced once. Each return link names the flowchart's heading in bold, so it still tells a reader where it goes when a screen reader lists links out of context. Every box carries a tooltip on its `click` line, the label assistive technology reads for that box. This follows *Style Guide: Navigation and Accessibility*.
...
|---------|--------|-------|
| 0.1.2 | Draft | Adopted the chart-heading rule from mermaid-navigation style guide v0.1.3: the flowchart sits under the heading "The Canonical Loop Flowchart" (matching its `accTitle`), the explicit `canonical-loop-chart` anchor is removed, the full description lives in `accDescr` only, and every return link now reads `[Return to the **The Canonical Loop Flowchart** section]`; accessibility notes updated to match |
