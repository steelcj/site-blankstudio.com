...
def main() -> None:
    ...
    # No families is a real answer, not a mistake: a site set in the reader's
    # own system fonts downloads nothing. The stylesheet is still written,
    # because locales.js points every locale's <link> at it and a missing file
    # is a 404 on every page. It simply declares no faces.
    if not families:
        note = (
            "/* GENERATED by scripts/branding/fetch-fonts.py from "
            "branding/<client>/fonts.yaml — do not edit by hand.\n"
            "   This site declares no webfonts: its type tokens name fonts the\n"
            "   reader already has, so there is nothing to download and nothing\n"
            "   to wait for. The file exists because every page links to it. */\n"
        )
        if args.dry_run:
            print(f"# would write {out['css_file']} declaring no @font-face rules\n")
            print(note)
            return
        os.makedirs(os.path.dirname(css_file), exist_ok=True)
        with open(css_file, "w", encoding="utf-8") as fh:
            fh.write(note)
        print(f"No families declared. Wrote {out['css_file']} with no @font-face rules.")
        return
